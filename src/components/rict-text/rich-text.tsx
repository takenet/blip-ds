...
export class RichText {
  ...
  componentDidLoad() {
    ...
    this.monitorElementContent(this.editor, (newValue) => {
      this.bdsChange.emit({ value: newValue });
    });
  ...
  protected treeElementsEditorChanged(value): void {
    ...
    const tagName = (tag) => tagList.includes(tag);
    const getLine = value.find((el) => el.classList.contains('line'));
    this.buttomBoldActive = tagName('b');
    this.buttomItalicActive = tagName('i');
    this.buttomStrikeActive = tagName('strike');
    this.buttomUnderlineActive = tagName('u');
    this.buttomCodeActive = tagName('code');
    ...
    this.buttomQuoteActive = tagName('blockquote');
    this.buttomH1Active = tagName('h1');
    this.buttomH2Active = tagName('h2');
    this.buttomH3Active = tagName('h3');
    this.buttomH4Active = tagName('h4');
    this.buttomH5Active = tagName('h5');
    this.buttomH6Active = tagName('h6');
  }
  ...
  private monitorElementContent(element: HTMLElement, callback: (newValue: string) => void) {
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'characterData' || mutation.type === 'childList') {
          callback(element.innerHTML); // ou innerText, dependendo do que precisa
        }
      });
    });

    observer.observe(element, {
      childList: true, // Observa mudanças na estrutura interna (como remoção/adição de elementos)
      subtree: true, // Monitora mudanças nos filhos do elemento
      characterData: true, // Detecta mudanças no texto interno
    });
  ...
  private wrapSelection(tag: string) {
    const selection = window.getSelection();
    if (!selection || selection.rangeCount === 0) return;
            ...
            <bds-grid ref={(el) => this.refButtonsListElement(el)} class="buttons-list" flex-wrap="wrap">
              <bds-tooltip tooltip-text={`${termTranslate(this.language, 'bold')}`} position="top-center">
                <bds-button
                  ...
                  disabled={!this.hasSelectionRange && !this.buttomBoldActive}
                  tabindex="-1"
                ></bds-button>
              </bds-tooltip>
              ...
              <bds-tooltip tooltip-text={`${termTranslate(this.language, 'link')}`} position="top-center">
                <bds-dropdown activeMode="click" position="bottom-left">
                  ...
                  <bds-grid padding="half" alignItems="center" gap="half" slot="dropdown-content">
                    <bds-input style={{ flexShrink: '99999' }} placeholder="adcione o link aqui"></bds-input>
                    <bds-button icon-left="check" onBdsClick={() => this.createLink}></bds-button>
                  </bds-grid>
                </bds-dropdown>
              </bds-tooltip>

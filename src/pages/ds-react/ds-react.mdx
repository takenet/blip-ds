...
Para utilizar o componente temos que realizar a importação pelo caminho ‘blip-ds-react/dist/components’, são wrappers gerados pela lib stencil/react-output-target, dessa maneira não será mais necessário a criação de wrappers na aplicação dos componentes em React.
...
import React from 'react';
import './App.scss';
import { BdsSelect } from 'blip-ds-react/dist/components';

import { DEFAULT_CHART_MARGIN } from "./chart.types";
 ...
 */
export function buildCategoryColorMap(data, categoryKey, palette = DEFAULT_LEGEND_PALETTE) {
 ...
 */
export function formatTick(value, formatter) {
    if (!formatter)
        return String(value);
 ...
 */
export function createBandScale(domain, range, padding = 0.5) {
    const [minRange, maxRange] = range;
...
export function getBandWidth(domain, range, padding = 0.5) {
    const [minRange, maxRange] = range;
    const totalWidth = maxRange - minRange;
    const bandCount = domain.length;
 ...
 */
export function generateLinePathMonotone(points, tension = 0.4) {
    if (points.length === 0)
        return '';
    if (points.length === 1) {
        const p = points[0];
 ...
 */
export function generateLinePathLinear(points) {
    if (points.length === 0)
        return '';
    let path = `M${points[0].x},${points[0].y}`;
    for (let i = 1; i < points.length; i++) {
 ...
 */
export function niceTickStep(rawStep) {
    if (rawStep <= 0)
        return 1;
...
export function calculateLineChartLayout(data, xKey, yKey, width, height, curve = 'monotone', margin = DEFAULT_CHART_MARGIN, tickCount = 5) {
...
export function calculateBarChartLayout(data, xKey, series, width, height, margin = DEFAULT_CHART_MARGIN, tickCount = 5, vertical = false, align = 'left') {
    const innerWidth = Math.max(0, width - margin.left - margin.right);
    const innerHeight = Math.max(0, height - margin.top - margin.bottom);
    const points = normalizeChartData(data);
    if (points.length === 0) {
...
export function calculateGridsAndLabels(data, xKey, yKey, width, height) {
    const points = normalizeChartData(data);
 ...
 */
export function calculateHeatmapLayout(data, xKey, yKey, valueKey, width, height, margin = DEFAULT_CHART_MARGIN, cellPadding = 2) {
    const points = normalizeChartData(data);
    if (points.length === 0) {
 ...
 */
export function generateDonutPath(startAngle, endAngle, outerRadius, innerRadius, padAngle = 0.02) {
 ...
 */
export function calculatePieLayout(data, labelKey, valueKey, width, height, innerRadiusPct = 60, palette = DEFAULT_LEGEND_PALETTE) {
    const points = normalizeChartData(data);
...
//# sourceMappingURL=chart-math.js.map

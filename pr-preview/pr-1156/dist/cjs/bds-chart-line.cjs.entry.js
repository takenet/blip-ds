...
const ChartLine = class {
    ...
    prepareLine(margin, xKey, yKey, tickCount = 5) {
        return chartMath.calculateLineChartLayout(this.data, xKey, yKey, this.actualWidth, this.actualHeight, this.curve, margin, tickCount);
    }
    emitLeave() {
        this.hoveredIndex = -1;
    ...
    render() {
        ...
        const lineLayouts = lines.map(line => chartMath.calculateLineChartLayout(this.data, xDataKey, line.dataKey, this.actualWidth, this.actualHeight, line.curve, margin, yTickCount));
        ...
        return (index.h(index.Host, null, index.h("svg", { width: this.actualWidth, height: this.actualHeight, class: "chart-line", "aria-hidden": "true" }, showHorizontalGrid && (index.h("g", { class: "chart-line__grid-y", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, yGridLines.map((y, idx) => (index.h("line", { key: `grid-y-${idx}`, x1: margin.left, y1: margin.top + y, x2: this.actualWidth - margin.right, y2: margin.top + y, stroke: gridStrokeColor, "stroke-width": "1" }))))), showVerticalGrid && (index.h("g", { class: "chart-line__grid-x", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, xGridLines.map((gridLine, idx) => (index.h("line", { key: `grid-x-${idx}`, x1: margin.left + gridLine.x, y1: margin.top, x2: margin.left + gridLine.x, y2: this.actualHeight - margin.bottom, stroke: gridStrokeColor, "stroke-width": "1" }))))), index.h("g", { transform: `translate(${margin.left}, ${margin.top})` }, lineLayouts.map((layout, lineIdx) => {
            ...
            const lineOpacity = this.activeLegendItem && !legendDataKey
                ...
                : 1;
            return (index.h("g", { key: `line-group-${lineIdx}`, style: { opacity: String(lineOpacity), transition: 'opacity 0.2s ease' } }, index.h("path", { class: "chart-line__path", d: layout.path, fill: "none", stroke: lines[lineIdx].color, "stroke-width": lines[lineIdx].strokeWidth }), lines[lineIdx].dot && layout.dots.map((dot, pointIdx) => {
                var _a;
                const dotOpacity = this.activeLegendItem && legendDataKey
                    ? (this.activeLegendItem === String(dot.datum[legendDataKey]) ? 1 : 0.2)
                    : 1;
                return (index.h("circle", { class: "chart-line__point", key: `point-${lineIdx}-${pointIdx}`, cx: dot.x, cy: dot.y, r: lines[lineIdx].radius, fill: categoryColorMap && legendDataKey
        ...
        }), lineLayouts.length > 0 && (index.h("line", { key: "hover-line", class: "chart-line__hover-line", x1: this.hoveredIndex >= 0 && lineLayouts[0].dots[this.hoveredIndex] ? lineLayouts[0].dots[this.hoveredIndex].x : 0, y1: 0, x2: this.hoveredIndex >= 0 && lineLayouts[0].dots[this.hoveredIndex] ? lineLayouts[0].dots[this.hoveredIndex].x : 0, y2: this.actualHeight - margin.top - margin.bottom, style: { stroke: 'var(--chart-hover-line-color)', opacity: this.hoveredIndex >= 0 ? '1' : '0', pointerEvents: 'none' }, "stroke-width": "1", "stroke-dasharray": "4,4" })), this.hoveredIndex >= 0 && lineLayouts.map((layout, lineIdx) => {
            const dot = layout.dots[this.hoveredIndex];
            if (!dot)
                return null;
            return (index.h("circle", { key: `hover-dot-${lineIdx}`, cx: dot.x, cy: dot.y, r: lines[lineIdx].radius, fill: lines[lineIdx].color, style: { stroke: 'var(--chart-hover-dot-stroke)', pointerEvents: 'none' }, "stroke-width": "2" }));
        })), showXAxisLine && (index.h("line", { class: "chart-line__x-axis-line", x1: margin.left, y1: this.actualHeight - margin.bottom, x2: this.actualWidth - margin.right, y2: this.actualHeight - margin.bottom, stroke: xLineColor, "stroke-width": "1" })), showYAxisLine && (index.h("line", { class: "chart-line__y-axis-line", x1: margin.left, y1: margin.top, x2: margin.left, y2: this.actualHeight - margin.bottom, stroke: yLineColor, "stroke-width": "1" })), showXLabels && chartAxisRender.renderXAxisLabels({
            xLabels,
            margin,
...
ChartLine.style = chartLineCss;

exports.bds_chart_line = ChartLine;
//# sourceMappingURL=bds-chart-line.entry.cjs.js.map

//# sourceMappingURL=bds-chart-line.cjs.entry.js.map
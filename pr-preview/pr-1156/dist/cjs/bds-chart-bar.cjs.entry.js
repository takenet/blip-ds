...
const chartBarCss = ".chart__x-labels{font-family:inherit;pointer-events:none}.chart__x-label{font-family:\"Nunito Sans\", \"Carbona\", \"Tahoma\", \"Helvetica\", \"Arial\", sans-serif;font-size:0.75rem;font-weight:400;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}.chart__y-labels{font-family:inherit;pointer-events:none}.chart__y-label{font-family:\"Nunito Sans\", \"Carbona\", \"Tahoma\", \"Helvetica\", \"Arial\", sans-serif;font-size:0.75rem;font-weight:400;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}:host{display:-ms-flexbox;display:flex;-ms-flex-direction:column;flex-direction:column;width:100%;height:100%;min-height:400px;max-width:1920px;max-height:1080px;--chart-hover-highlight-color:rgba(0, 0, 0, 0.08)}svg{-ms-flex:1 1 0px;flex:1 1 0;min-height:0;width:100%}.chart-bar__grid{pointer-events:none}.chart-bar__rect{-webkit-transition:opacity 0.2s ease, -webkit-transform 0.2s ease;transition:opacity 0.2s ease, -webkit-transform 0.2s ease;transition:transform 0.2s ease, opacity 0.2s ease;transition:transform 0.2s ease, opacity 0.2s ease, -webkit-transform 0.2s ease;cursor:default}.chart-bar__rect:hover{-webkit-transform:translateY(-4px);transform:translateY(-4px);opacity:0.9}";
...
const ChartBar = class {
    ...
    componentDidRender() {
        ...
        const categoryColorMap = legendDataKey
            ? chartMath.buildCategoryColorMap(this.data, legendDataKey)
            : null;
    ...
    prepareBars(series, margin, xKey, tickCount = 5) {
        return chartMath.calculateBarChartLayout(this.data, xKey, series, this.actualWidth, this.actualHeight, margin, tickCount, this.vertical, this.align);
    }
    emitLeave() {
        this.hoveredIndex = -1;
    ...
    render() {
        ...
        const margin = this.computeMargin(config);
        const { bars, xLabels, yLabels, yGridLines, xGridLines } = this.prepareBars(series, margin, xDataKey, yTickCount);
        ...
        return (index.h(index.Host, null, index.h("svg", { width: this.actualWidth, height: this.actualHeight, class: "chart-bar", "aria-hidden": "true" }, this.vertical && showVerticalGrid && (index.h("g", { class: "chart-bar__grid-x", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, xLabels.map((label, idx) => (index.h("line", { key: `grid-cat-${idx}`, x1: margin.left, y1: margin.top + label.x, x2: this.actualWidth - margin.right, y2: margin.top + label.x, stroke: gridStrokeColor, "stroke-width": "1" }))))), this.vertical && showHorizontalGrid && (index.h("g", { class: "chart-bar__grid-y", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, yLabels.map((label, idx) => (index.h("line", { key: `grid-val-${idx}`, x1: margin.left + label.y, y1: margin.top, x2: margin.left + label.y, y2: this.actualHeight - margin.bottom, stroke: gridStrokeColor, "stroke-width": "1" }))))), !this.vertical && showHorizontalGrid && (index.h("g", { class: "chart-bar__grid-y", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, yGridLines.map((y, idx) => (index.h("line", { key: `grid-y-${idx}`, x1: margin.left, y1: margin.top + y, x2: this.actualWidth - margin.right, y2: margin.top + y, stroke: gridStrokeColor, "stroke-width": "1" }))))), !this.vertical && showVerticalGrid && (index.h("g", { class: "chart-bar__grid-x", "stroke-dasharray": getStrokeDasharray(gridStrokeStyle) }, xGridLines.map((gridLine, idx) => (index.h("line", { key: `grid-x-${idx}`, x1: margin.left + gridLine.x, y1: margin.top, x2: margin.left + gridLine.x, y2: this.actualHeight - margin.bottom, stroke: gridStrokeColor, "stroke-width": "1" }))))), index.h("g", { transform: `translate(${margin.left}, ${margin.top})` }, index.h("g", { class: "chart-bar__bars" }, bars.map((bar, index$1) => {
            var _a, _b;
            ...
            const barOpacity = this.activeLegendItem
                ...
                : 1;
            return (index.h("rect", { key: `bar-${index$1}`, class: "chart-bar__rect", x: bar.x, y: bar.y, width: Math.max(bar.width, 0), height: Math.max(bar.height, 0), rx: sc.radius, fill: categoryColorMap && legendDataKey
            ...
            return (index.h("rect", { key: `hover-bg-${hi}`, class: "chart-bar__hover-bg", x: b.x, y: b.y, width: Math.max(b.width, 0), height: Math.max(b.height, 0), rx: sc.radius, style: { fill: 'var(--chart-hover-highlight-color)', pointerEvents: 'none' } }));
        })), showXAxisLine && (index.h("line", { class: "chart-bar__x-axis-line", x1: margin.left, y1: this.vertical ? margin.top : this.actualHeight - margin.bottom, x2: this.vertical ? margin.left : this.actualWidth - margin.right, y2: this.actualHeight - margin.bottom, stroke: xLineColor, "stroke-width": "1" })), showYAxisLine && (index.h("line", { class: "chart-bar__y-axis-line", x1: margin.left, y1: this.vertical ? this.actualHeight - margin.bottom : margin.top, x2: this.vertical ? this.actualWidth - margin.right : margin.left, y2: this.actualHeight - margin.bottom, stroke: yLineColor, "stroke-width": "1" })), showXLabels && (this.vertical ? (
        // Vertical mode: category labels on left — unique layout, rendered inline
        index.h("g", { class: "chart-bar__x-axis", style: { pointerEvents: 'none' } }, xLabels.map((label, idx) => (index.h("text", { key: `x-axis-${idx}`, "text-anchor": "end", fill: xLabelColor, "font-weight": idx === this.hoveredIndex ? 'bold' : 'normal', x: margin.left - 8, y: margin.top + label.x + 4, class: "chart__x-label" }, chartMath.formatTick(label.label, xTickFormatter)))))) : chartAxisRender.renderXAxisLabels({
            xLabels,
            margin,
        ...
        index.h("g", { class: "chart-bar__y-axis", style: { pointerEvents: 'none' } }, yLabels.map((label, idx) => (index.h("g", { key: `y-axis-${idx}` }, showYTickLine && (index.h("line", { x1: margin.left + label.y, y1: this.actualHeight - margin.bottom, x2: margin.left + label.y, y2: this.actualHeight - margin.bottom + 6, stroke: yLineColor, "stroke-width": "1" })), index.h("text", { "text-anchor": "middle", fill: yLabelColor, x: margin.left + label.y, y: this.actualHeight - margin.bottom + 6 + yTickMargin, class: "chart__y-label" }, chartMath.formatTick(label.label, yTickFormatter))))))) : chartAxisRender.renderYAxisLabels({
            yLabels,
            margin,
...
ChartBar.style = chartBarCss;

exports.bds_chart_bar = ChartBar;
//# sourceMappingURL=bds-chart-bar.entry.cjs.js.map

//# sourceMappingURL=bds-chart-bar.cjs.entry.js.map
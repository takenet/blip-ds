  ...
  renderChips() {
    ...
    return this.internalChips.map((chip, index$1) => {
      ...
      if (chip.length <= limit) {
        return (index.h("bds-chip-clickable", { id: id, key: id, color: "outline", close: !this.disabled, onChipClickableClose: (event) => this.removeChip(event), dtButtonClose: this.dtButtonClose, exportparts: "chip" }, chip));
      }
      else {
        return (index.h("bds-tooltip", { key: id, position: "top-center", "tooltip-text": chip, exportparts: "chip" }, index.h("bds-chip-clickable", { id: id, key: id, color: "outline", close: !this.disabled, onChipClickableClose: (event) => this.removeChip(event), dtButtonClose: this.dtButtonClose, exportparts: "chip" }, `${chip.slice(0, limit)}...`)));
      }
    });
  }
  renderIcon() {
